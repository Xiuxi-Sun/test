...
DOCUMENTATION = '''
---
module: azure_rm_expressroutecircuitconnection_info
version_added: '2.9'
short_description: Get ExpressRouteCircuitConnection info.
description:
  - Get info of ExpressRouteCircuitConnection.
options:
  resource_group_name:
    description:
...
EXAMPLES = '''
    - name: ExpressRouteCircuitConnectionGet
      azure_rm_expressroutecircuitconnection_info: 
...
express_route_circuit_connections:
  description: >-
    ...
    ExpressRouteCircuitConnection and the values are the facts for that
  ...
  contains:
    ...
    provisioning_state:
      description:
        - >-
          The provisioning state of the express route circuit connection
          resource.
      returned: always
      type: str
      sample: null
...
class AzureRMExpressRouteCircuitConnectionInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMExpressRouteCircuitConnectionInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_group_name is not None and
            ...
            self.results['express_route_circuit_connections'] = self.format_item(self.get())
        elif (self.resource_group_name is not None and
              self.circuit_name is not None and
              self.peering_name is not None):
            self.results['express_route_circuit_connections'] = self.format_item(self.list())
        return self.results
    ...
    def list(self):
        ...
        try:
            response = self.mgmt_client.express_route_circuit_connections.list(resource_group_name=self.resource_group_name,
                                                                               circuit_name=self.circuit_name,
                                                                               peering_name=self.peering_name)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
...
def main():
    AzureRMExpressRouteCircuitConnectionInfo()
