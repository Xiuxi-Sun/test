...
DOCUMENTATION = '''
---
module: azure_rm_virtualmachinescalesetvmruncommand
version_added: '2.9'
short_description: Manage Azure VirtualMachineScaleSetVMRunCommand instance.
description:
  - >-
...
options:
  ...
  state:
    description:
      - Assert the state of the VirtualMachineScaleSetVMRunCommand.
      - >-
        Use C(present) to create or update an VirtualMachineScaleSetVMRunCommand
...
EXAMPLES = '''
    - name: Create VirtualMachineScaleSet VM run command.
      azure_rm_virtualmachinescalesetvmruncommand: 
        instance_id: '0'
    ...
    - name: Delete VirtualMachineScaleSet VM run command.
      azure_rm_virtualmachinescalesetvmruncommand: 
        instance_id: '0'
        resource_group_name: myResourceGroup
        run_command_name: myRunCommand
...
class AzureRMVirtualMachineScaleSetVMRunCommand(AzureRMModuleBaseExt):
    def __init__(self):
        ...
        super(AzureRMVirtualMachineScaleSetVMRunCommand, self).__init__(derived_arg_spec=self.module_arg_spec,
    ...
    def create_update_resource(self):
        try:
            response = self.mgmt_client.virtual_machine_scale_set_vmrun_commands.create_or_update(resource_group_name=self.resource_group_name,
            ...
            self.log('Error attempting to create the VirtualMachineScaleSetVMRunCommand instance.')
            self.fail('Error creating the VirtualMachineScaleSetVMRunCommand instance: {0}'.format(str(exc)))
        return response.as_dict()

    def delete_resource(self):
        try:
            response = self.mgmt_client.virtual_machine_scale_set_vmrun_commands.delete(resource_group_name=self.resource_group_name,
                                                                                        vm_scale_set_name=self.vm_scale_set_name,
                                                                                        instance_id=self.instance_id,
                                                                                        run_command_name=self.run_command_name)
        except CloudError as e:
            self.log('Error attempting to delete the VirtualMachineScaleSetVMRunCommand instance.')
            self.fail('Error deleting the VirtualMachineScaleSetVMRunCommand instance: {0}'.format(str(e)))
...
def main():
    AzureRMVirtualMachineScaleSetVMRunCommand()
