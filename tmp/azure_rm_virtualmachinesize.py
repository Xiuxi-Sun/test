...
class AzureRMVirtualMachineSize(AzureRMModuleBaseExt):
    def __init__(self):
        ...
        super(AzureRMVirtualMachineSize, self).__init__(derived_arg_spec=self.module_arg_spec,
    ...
    def create_update_resource(self):
        try:
            ...
            self.log('Error attempting to create the VirtualMachineSize instance.')
            self.fail('Error creating the VirtualMachineSize instance: {0}'.format(str(exc)))
        return response.as_dict()

    def delete_resource(self):
        try:
            response = self.mgmt_client.virtual_machine_sizes.delete()
        except CloudError as e:
            self.log('Error attempting to delete the VirtualMachineSize instance.')
            self.fail('Error deleting the VirtualMachineSize instance: {0}'.format(str(e)))
