...
class AzureRMSubscription(AzureRMModuleBaseExt):
    ...
    def create_update_resource(self):
        try:
            if self.to_do == Actions.Create:
                response = self.mgmt_client.subscription.create()
            else:
                response = self.mgmt_client.subscription.update()
    ...
    def delete_resource(self):
        try:
            response = self.mgmt_client.subscription.delete()
    ...
    def get_resource(self):
        try:
            response = self.mgmt_client.subscription.get()
        except CloudError as e:
            return False
        return response.as_dict()
